...
contract('[PlasmaMVP] Block Submissions', async (accounts) => {
    ...
    it("Submit block from authority", async () => {
        ...
        let tx = await instance.submitBlock([header], [1], 1, {from: authority});
        ...
        assert.equal((await instance.childChain.call(1))[0], header, 'child block merkle header does not match submitted merkle header.');
    });

    it("Submit block from someone other than authority", async () => {
        let prev = (await instance.lastCommittedBlock.call()).toNumber();

        let [err] = await catchError(instance.submitBlock([web3.utils.keccak256('578484785954')], [1], 1, {from: accounts[1]}));
        if (!err)
    ...
    it("Can submit more than one merkle header", async () => {
        ...
        await instance.submitBlock([toHex(header1), toHex(header2)], [1, 2], 1, {from: authority});

        assert.equal((await instance.lastCommittedBlock.call()).toNumber(), 2, "lastCommittedBlock not incremented incorrectly");
        assert.equal((await instance.childChain.call(1))[0], toHex(header1), "mismatch in block header");
        assert.equal((await instance.childChain.call(2))[0], toHex(header2), "mismatch in block header");
    });
    ...
    it("Enforces block number ordering", async () => {
        ...
        await instance.submitBlock([toHex(header1)], [1], 1, {from: authority});
        let err;
