...
// creates a block with a single input tx and returns all the relevant information
let sendSingleInputTx = async function(rootchain, authority, txBytes, sender) {
    let txHash = web3.sha3(txBytes.toString('hex'), {encoding: 'hex'});
  
    // create signature by deposit owner. Second signature should be zero
    let signatures = await web3.eth.sign(sender, txHash);
    signatures += Buffer.alloc(65).toString('hex');

    // include this transaction in the next block
    let merkleHash = web3.sha3(txHash.slice(2) + signatures.slice(2), {encoding: 'hex'});
    console.error("hello there");
    //let result = generateMerkleRootAndProof([merkleHash], 0);
    let result = generateMerkleRootAndProof(merkleHash, 0);
    console.error("general kenobi");
    let root = result[0];
    for (let i = 0; i < 16; i++)
    ...
    let confirmationHash = web3.sha3(merkleHash.slice(2) + root, {encoding: 'hex'});
    let confirmationSignatures = await web3.eth.sign(sender, confirmationHash);
    confirmationSignatures += Buffer.alloc(65).toString('hex'); // empty second confirmsig

    return [blockNum, signatures, confirmationSignatures, result[1]];
}; 
